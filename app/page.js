...
export default function YorisoiApp() {
  ...
  const getInviteUrl = () => {
    ...
    return `${window.location.origin}?code=${pairCode}`;
  };
  ...
  const editPlanItem = (type) => {
    ...
    const label = type === 'doing' ? '今の状態' : type === 'requests' ? 'お願い' : '遠慮';
    const newValue = window.prompt(`${label}を入力`, current);
    if (newValue !== null) {
  ...
  const addCustomOption = (type) => {
    const newItem = window.prompt("追加したい項目を入力");
    ...
    <div style={pageStyle}>
      {isSetting ? (
        <div style={{ background: '#fff', padding: '30px 20px', borderRadius: '30px' }}>
          ...
          <button onClick={logout} style={{ width: '100%', marginTop: '30px', color: '#f87171' }}>ログアウト</button>
        </div>
      ) : (
        <>
          ...
          {role === 'him' ? (
            <div>
              {status && (status.symptoms?.length > 0 || status.level !== undefined) ? (
                <>
                  ...
                  <div className="info-card" style={{ borderLeft: '6px solid #ff9eb5', padding: '20px', background: '#fff', borderRadius: '25px' }}>
            ...
            <>
              ...
              <div style={{ background: '#fff', padding: '20px', borderRadius: '20px', marginBottom: '30px' }}>
                <div onClick={() => editPlanItem('requests')} style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>📋 お願い内容</span><Edit3 size={18} />
                </div>
                <div style={{ marginTop: '10px' }}>{status?.requests?.join('、') || "未設定"}</div>
              </div>
              <button 
      ...
      <style jsx>{`
        ...
        .chip { padding: 12px 18px; border-radius: 20px; background: #fff; font-size: 14px; }
        ...
        .quick-reply { padding: 12px; background: #fff; border: 1px dashed #9ebbd7; border-radius: 12px; font-size: 12px; color: #9ebbd7; }
